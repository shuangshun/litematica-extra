...
public class NbtStreamReader {
    ...
    public NbtStreamReader(InputStream inputStream) {
    ...
    public NbtScanResult scanRootFields(String... fieldNames) throws IOException {
        ensureRootParsed();

        NbtScanResult result = new NbtScanResult();
        Set<String> targetFields = Set.of(fieldNames);
        ...
        switch (tagId) {
            ...
            case TAG_LIST: {
                ...
                for (int i = 0; i < length; i++) {
                    NbtElement element = readFullTag(listType);
                    if (element != null) {
                        list.add(element);
                    }
                }
